...
function fixClassificationSheet_(ss, fixes) {
  ...
  if (numRows > 0) {
    ...
    for (var j = 0; j < numRows; j++) {
      ...
      if (String(types[j][0] || '').toLowerCase().indexOf('perman') === -1) {
        sheet.getRange(cfg.CLASSIFICATION_DATA_START_ROW + j, cfg.TYPE_COLUMN).setValue('Permanent');
        retyped++;
      }
    }
    if (retyped > 0) fixes.push('Re-typed ' + retyped + ' misclassified permanent parameter(s) in column B');
  }
}
...
function fixCallsDataHeaders_(ss, fixes) {
  ...
  var missingHeaders = expected.filter(function (h) { return headers.indexOf(h) === -1; });
